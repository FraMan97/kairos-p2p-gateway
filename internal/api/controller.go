...
func PutFile(w http.ResponseWriter, r *http.Request) {
	...
	if clientHash != "" {
		go func(h string) {
			targetURL := fmt.Sprintf("%s/anchor", config.AppConfig.KairosLedgerURL)
			payload := fmt.Sprintf(`{"hash":"%s"}`, h)

			resp, err := http.Post(targetURL, "application/json", strings.NewReader(payload))
			if err != nil {
				log.Printf("[PutFile] Ledger anchoring failed for hash %s: %v", h, err)
				return
			}
			defer resp.Body.Close()
			log.Printf("[PutFile] Ledger anchoring dispatched for hash %s. Ledger status: %d", h, resp.StatusCode)
		}(clientHash)
	}
...
func GetFile(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("[GetFile] Proxying request for fileId: %s to Kairos Network", fileId)

	targetURL := fmt.Sprintf("%s/get?id=%s", config.AppConfig.KairosNetworkURL, fileId)
	resp, err := http.Get(targetURL)
	if err != nil {
		log.Printf("[GetFile] Error connecting to Kairos Network: %v", err)
	...
	if resp.StatusCode != http.StatusOK {
		log.Printf("[GetFile] Upstream error status: %d", resp.StatusCode)
		w.WriteHeader(resp.StatusCode)
		io.Copy(w, resp.Body)
		return
	}
	...
	if err != nil {
		log.Printf("[GetFile] Error streaming file to client: %v", err)
		return
	}

	log.Printf("[GetFile] Successfully streamed %d bytes for fileId: %s", bytesCopied, fileId)
}
...
func GetReceipt(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		log.Printf("[GetReceipt] Invalid method %s", r.Method)
		http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
		return
	}
	...
	log.Printf("[GetReceipt] Proxying receipt request for hash: %s to Ledger", hash)

	targetURL := fmt.Sprintf("%s/receipt?hash=%s", config.AppConfig.KairosLedgerURL, hash)
	resp, err := http.Get(targetURL)
	if err != nil {
		log.Printf("[GetReceipt] Error contacting ledger: %v", err)
